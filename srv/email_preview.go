...
func emailPreviewFixtures(base string) []emailPreview {
	...
	return []emailPreview{
		...
		{Kind: "transmittal_update", Subject: "Transmittal Updated: " + tx.Book.Title + " (mike-casey)", Text: buildTxNotifyText(snap.ProjectName, "mike-casey", tx.Book.Title, tx.Book.Author, "draft", adminLoginURL(base, "/mike-casey/casey-001/factory/#transmittal")), HTML: buildTxNotifyHTML(snap.ProjectName, "mike-casey", tx.Book.Title, tx.Book.Author, "draft", adminLoginURL(base, "/mike-casey/casey-001/factory/#transmittal"))},
