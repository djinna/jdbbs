...
type BookMap struct {
	...
	Errors        []string          `json:"errors,omitempty"` // the build would produce an empty or wrong book (high)
...
func buildBookMap(paras []docxPara, untitledNames []string, bookTitle, bookAuthor string) *BookMap {
	...
	if m.KeptWords() == 0 {
		m.Errors = append(m.Errors, "This file has no body text: everything in it is dropped as title, byline or contents, so a proof would be only the generated pages and a final would waste a credit (finals are refused until this is fixed). Put the book's text under Heading 1 section heads and upload again.")
	}
	...
	for i, sp := range spans {
		if kinds[i] == "" && isContentsHeading(sp.title) {
			...
			if isTitleStub(sp.paras) {
				...
				// More than a typed table of contents sits under it.
				m.Warnings = append(m.Warnings, fmt.Sprintf("“%s” and the %s under it (%s) are dropped because the contents page is generated by the build — that is more than a typed contents list. If there is real text under this heading, give it its own Heading 1.", sp.title, plural(len(sp.paras), "paragraph"), previewParas(sp.paras)))
			}
		}
	}
