...
| 2026-09-11 | **Client-visible on-disk documents.** `pi-client/{slug}/` root + `serveClientDoc` with `checkClientAuth`. Recipe is in the standard §7; build when the first real client doc arrives. | Deferred deliberately — no queued need. Re-parked 2026-09-19 (punch list 6.3): trigger stays "first real client doc". |
...
| 2026-09-18 | **Plantin MT Pro Bold missing on the VM.** `typesetting/fonts/licensed/plantin-mt-pro/` has Regular + Italic only, so `strong` in body text silently sets regular in every Plantin print build (found testing Glossary Entry). Needs the Bold (and Bold Italic) OTF dropped in — Jenna's licence, never in git. | Check before Ghosts/Obliquities reprint; workshop week. |
...
## Done (struck 2026-09-18 / 2026-09-19)

| date | idea | landed |
|---|---|---|
| 2026-09-19 | **Per-project interactive style sheet** (was "per-client stylesheet instances", 2026-09-11). Each project gets its own accept/reject/edit/add sheet seeded from `house_style`, at `/{client}/{project}/stylesheet/`. | Punch list 6.1 B, built 2026-09-19; see `scratch/briefs/stylesheet-6.1-2026-09-19.md` → `docs/reviews/SESSION-HANDOFF-2026-09-18.md` addendum 9. |
