...
- **Live byline test passed on `79d5424` (after the style-Div fix below):** a synthetic anthology DOCX (uploaded to a throwaway project, then deleted — prod left clean) detected all three byline forms end-to-end — `"By Spencer Nitkey"`→Spencer Nitkey (By-prefix), `*Claire Pichelin*`→Claire Pichelin (italic), `custom-style="Author"`→Daniel Fernández (Word style) — plus the no-byline intro → title-only. `count: 4`.
- Not yet done (optional, non-blocking): Twitter Years empty-banner spot-check (expected `count: 0`, needs that book spun up in its own session); SPA banner/Apply UI dogfood (backend fully verified via API; live visual check needs admin-session cookies imported into the headless browser).

**Follow-up fix 2026-05-31 (found by the live synthetic-byline test):** `pandoc --from=docx+styles` wraps *every* paragraph in a Div tagged with its Word paragraph style ("First Paragraph", "Body Text", "Author", …), so byline paragraphs are **never** top-level `Para`s on real DOCX input — only headings are emitted bare. The original `scanByline` checked the "By X" / italic heuristics on top-level `Para`s only, so on real DOCX it would have caught *only* the explicit `custom-style="Author"` case and silently missed By-prefix + italic bylines (the markdown-shaped unit tests didn't catch this — they used bare `Para`s). Fixed: new `collectCandidates` descends through style Divs, surfacing inner `Para`s tagged with their nearest custom-style; `scanByline` now ranks those (Author-style > By-prefix > italic). Validated by porting the fixed walk to Python and running it on a real `docx+styles` AST (By/italic/Author/title-only all correct), plus 6 new docx-shaped table tests. This is exactly the bug the live byline test existed to surface.
...
**TRK-DESIGN-002 fully done (DESIGN-002 fan + DESIGN-001 `4512aac`):** print-only licensed-font wiring. Reused the existing `typography.body_font`/`heading_font` selects (already print-only + auto-list `licensed/` fonts) instead of adding the proposed `pdf.*` fields. Shipped (commit `8633883`): gitignored `licensed/` (+ tracked README), `scripts/sync-licensed-fonts.sh`, EPUB `/licensed/` guard (`epubEmbedFontArgs` + tests), body-font fallback chain, font categorization, admin note. **User-side step now complete via `4512aac`:** the real Plantin MT Pro + Proxima Nova OTF are installed in `licensed/` (gitignored — only README tracked), synced to the VM, and set as the `series-template.typ` defaults — so the plumbing is exercised end-to-end. Re-verified 2026-06-01: fonts `check-ignore`d, `go build`/`vet` + guard tests pass. Remaining is the print-compile check on the VM (`pdffonts` shows subsetted Plantin; EPUB shows zero).
...
Verified locally: `go build`/`go vet`/`gofmt`/`shellcheck` clean; new guard tests + existing EPUB packaging test pass; gitignore confirmed (font ignored, README tracked). EPUB CSS (`buildCSS`) confirmed OFL-only (no Plantin/Proxima).

**User-side step — COMPLETE via TRK-DESIGN-001 (commit `4512aac`, 2026-06-01).** The real Plantin MT Pro + Proxima Nova OTF are now in `typesetting/fonts/licensed/{plantin-mt-pro,proxima-nova}/` (gitignored — `git ls-files typesetting/fonts/licensed/` shows only `README.md`), synced to the VM, and wired as the `series-template.typ` defaults (`body-font: "Plantin MT Pro"`, `heading-font: "Proxima Nova"`) plus the 9 Ghosts chapter files. So the whole DESIGN-002 plumbing — gitignored `licensed/` tree, `sync-licensed-fonts.sh`, the EPUB `/licensed/` guard, the admin font menu, and the body-font fallback chain — is now exercised end-to-end on real licensed fonts. Re-verified 2026-06-01 at `4512aac`: both `.otf` families `git check-ignore`-confirmed ignored; `go build`/`go vet` clean; `epubEmbedFontArgs` guard tests pass; fallback chain intact (`series-template.typ:491`). Remaining checks belong to the print compile (VM): `pdffonts <pdf>` shows subsetted Plantin embedded; `unzip -l <epub>` + CSS grep show zero Plantin. Confirm the licenses are perpetual-desktop (not subscription) before relying on archival PDFs.
