...
## Addendum 18 (2026-09-20, ~13:20 UTC) — close of conv cRARHD4; next conversation starts here

Since addendum 17 (all pushed):
- **0.29 landed** (variant B, `4bdea44`): `h2-above 24pt / h2-below 16.625pt / h3-above 14pt / h3-below 7.25pt`; slots round to whole body lines (11.625 pt) so spreads align; sampler rebuilt; TYPOGRAPHY.md updated (+ "no consecutive heads" guideline). Spread comps on the item. After workshop: half-gap for consecutive heads; "Heading 2 (A-head)" naming pass.
- **Runpage**: contents box (counts + open item ids per section), collapsible sections (per-device), folded done items, folded note threads (`scripts/runpage/page.html`). Note: "0.x" items keep their Inbox number after triage, so 0.9 lives in §4.
- **0.9A — three factories exist, tokens minted, verified from outside** (no token → 401, Bearer → 200): pinstitute = project 22 (pass 7, 1 credit left; **never build here**), vgr = project 7 *The Twitter Years* (new pass 13), monstrous = project 28 *Perception Must Preserve* (was `snitkey`, renamed; old URL 301s; pass 11). Tokens: `scratch/customer-tokens/project-{22,7,28}.token` (gitignored). Client display name for monstrous is still "Sachin" — Jenna hasn't said.
- **Jenna's plan for 0.9**: she will NOT send anything to the three customers. She wants to play all three herself ("set up three test profiles locally and run their books through myself") to see what it takes before asking them to click around. **She asked for an instruction doc — the thing we would hand a customer — as a GitHub page for now, and she'll work through it today.** Not written yet. Inputs: `docs/API-CLI-RECIPE-2026-09-19.md` (verified endpoints/shapes), `/factory/api` (public page), `scripts/factory-cli.py`, `scripts/factory-demo.sh`, the portal path `/{client}/{project}/factory/`. Client passwords: all three have one set; reset via `POST /api/admin/clients/{slug}/password` if she needs to sign in as them (mcheck's is in `scratch/idx-smoke/pw`). Open question to ask her first: "locally" = browser on her Mac against jdbbs.exe.xyz as the three clients (the accounts live on the VM), or the Mac app (own DB, no passes)? Assume the former unless she says otherwise.
- **Hotfix shipped** (`2180f97`): client rename failed with FOREIGN KEY error since `login_links` arrived; fixed with `PRAGMA defer_foreign_keys` + carrying `login_links`/`factory_events`; regression test.
- **/factory redeem form**: mouse type under "Your name" — one pass = one account + one book; account named after the author (or you), holds later books; imprints say so in the title field (jdbbs-public `187691c`). Same wording for the Buy/Stripe section still to add after she reads it.
- **5.22**: Jenna likes both ideas (high-graded Google list; by-request MyFonts purchases added to the studio library). Reply posted with an 8-face OFL shortlist + criteria, a 6-face by-request shortlist (Sabon Next, Bembo Book, Dante, Janson Text, Miller Text, Swift/Scala), pricing shape (~$150 request fee, +$50 library), UI = three groups on the Typeface row. Awaiting her yes → then write memo §6; OFL drop-in after workshop.
...
## Addendum 23 (2026-09-22, ~14:05 UTC) — close of conv cR3V2U7 (workshop Tue, live); next conversation starts here

**Shipped (all pushed, three restarts, zero builds converting each time):**
- `9258e56` **5.25 slices 1–3** — `srv/customstyles.go` (new): `custom_styles[]` normalised with `based_on` (13 factory paragraph styles; character: none/italic/small caps/code), `indent` 0–2 only for Verse/Block Quote/Code Block, `space_before` 1/3/6 only for Section Break; precedence hand-written snippet → designed preset → parent + deltas; `customStyleResolvedForm` ("verse2 → Verse, indent one level"); admin preset/snippet carried forward by name on transmittal re-pull; `normalizeSpecCustomStyles` on admin PUT. `typstStyleIdent` now keyword-safe (`break` → `break-style`; **this was Fotis's book-57 failure** — `#let break(content)` in our generated config, error text pointed him at his manuscript). `series-template.typ`: `poem(indent:)`, `blockquote(indent:)`, `code-block(indent:)`, `section-break-gap(gap:)` (+ `section-break = section-break-gap()`). Lua filter: declared Verse/Signature-based styles coalesce (`coalesce` flag on `pandocStyle`). Transmittal row UI: Name · Type · Based on · Indent/Space before · Purpose + "In the book:" line. Tests `srv/customstyles_test.go`; full `go test ./srv/` green.
- `afcc542` **0.33** — sticky strip shows book title (left, small) + `v MMDD.hash` (right; `GET /api/version`, Makefile `-ldflags -X srv.exe.dev/srv.BuildVersion`); `tx:title` event; server mirrors transmittal title → `projects.name` on save for pass projects.
- `af4a031` **0.30** — header takes transmittal title from first paint; one-off DB rename of projects 30/32/33 to their transmittal titles (7/18/22/24 left to sync on their next save).
- Punch list: 0.33, 0.30 ticked; 0.34 (onboarding: "styles are names") parked in §5; 5.25 `[~]`; export written `docs/runs/PUNCHLIST-2026-09-22.md`.

**Verified in browser on mcheck (17):** `verse2` (Verse, 1), `verse3` (Verse, 2), `break2` (Section Break, 3×) are **declared and saved** on project 17's transmittal. **Not yet done:** upload `scratch/verse/in.docx`, Inspect, proof, read the PDF pages (pdftoppm + read_image) — that is the next action. Also not yet checked: `templates/styles.typ` is a separate file — confirm `sc` is in scope where the generated `#let x(content) = sc(content)` lands (character parent "small caps"), else use `smallcaps(content)`.

**Remaining 5.25/5.26 slices:** (4) EPUB classes in `docx-to-epub.lua` + `epub-styles.css` (`verse-indent-1/2`, gap); Word template `basedOn` + indent in `generate-word-template.py` (reads `based_on`/`indent` from spec JSON); (5) inline character markers in `apply-style-markers.py` + `detect-edge-cases.py`, friendlier unresolved wording, `TestApplyStyleMarkersScript` cases; (6) admin book-spec panel (`admin.html` ~L2725 `tsRenderCustomStyles`, already has preset select + typst textarea) gains Based on/Indent/Space before + resolved line; (7) `/factory#bring`, `/workshop#bring`, WORD-FREE-AUTHORING note.

**Fotis review:** subagent `fotis-review` (conv cFLT76L) ran long and had not written `docs/reviews/FOTIS-HELO-WORD-REVIEW-2026-09-22.md` at handoff; brief is `scratch/fotis/BRIEF.md`, his files in `scratch/fotis/`. If the file appears, read its summary section only and commit it; if not, re-run with a cheaper model and a hard instruction to write the file first, then investigate. Root cause is already fixed (keyword ident). Still open from his file: build-failed text must distinguish "our generated config" from "your text" (5.24), inline `[[ITAL]]`/`[[BOLD]]` leaked into book.typ (5.26), and ~9 of his 21 declarations duplicate factory styles.

**Why this session burned context fast (asked by Jenna):** (1) two large uploads were inspected inline (Fotis's spec JSON dump ≈ 3k tokens, a 21-style census) before deciding to delegate — decide *delegate vs. inline* before the first read of any attachment; (2) three interleaved streams (5.25 build, 0.33/0.30 hotfixes, Fotis triage) each cost re-orientation reads of the same files (`bookspecs.go`, `transmittal.js`, `factory.js`); (3) several `sed -n` windows of 60–150 lines where a `rg -n` for one symbol would have done; (4) the subagent brief was written in full into the chat (≈1.5k tokens) — write it to a file with a heredoc and keep the prompt to one line (I did the file part, but the heredoc still echoes). Rule going forward: one stream per session during the freeze; hotfixes from the punch list go to a second short session.

Metrics: ~62 % context at handoff; 0 files read in full via guard bypass.

## Addendum 24 (2026-09-22, ~14:40 UTC) — close of conv cRGLCIJ (workshop Tue, live); next conversation starts here

**Shipped (all pushed; two restarts, 0 builds converting each time; version strip shows 0922.5b7ba69):**
- `5b7ba69` **0.37** Style sheet link first on the step-1 transmittal rule (`headerActions` in `transmittal.js`; cache-bust `?v=20260922a`). House sheet is public at `/stylesheet/`; each book sheet links back to it.
- `39397cd` **0.38** Portal Recent activity: `GET /api/clients/{client}/factory-log` (`srv/factory_events.go`, `handleClientFactoryLog`, login.* hidden, actor → you/studio/factory); `client.html` Files tab → Factory tab, rows link to the factory step. Journal unchanged. The client digest email still says "file transfers and journal" (`client.html` ~L894) — wording only.
- `427013b` **0.39 hotfix** (root cause of the 5.25 "declarations missing"): `storeChapterSuggestions` (`chapter_detect.go`) and `UpdateBookSpecCover` (`db/queries/book_specs.sql` + hand-edited `db/dbgen/book_specs.sql.go`, no sqlc on the VM) no longer bump `book_specs.updated_at`, which is the gate in `syncSpecFromTransmittal` (`bookspecs.go:1002`). Regression test `TestIncidentalSpecWritesDoNotBlockTransmittalRePull`. One-off: project 17's stale spec stamp reset by SQL. Other attendee projects checked by the diag subagent: 30 in sync (21/21), 29 draft, 31/33 no declarations. Diagnosis: `scratch/5.25/DIAG-REPORT.md`.
- `23258f6` punch list 4 archived → `docs/runs/PUNCHLIST-2026-09-22-list4.md` (the earlier `PUNCHLIST-2026-09-22.md` removed as a duplicate); **list 5** live and short.
...
- 3.2 (15:15 UTC): deck slide 2 got the pages/scrolls seesaw line + archive.org speaker note (option A; jdbbs-public 2772494). Then (15:30 UTC) Jenna asked for the archive.org grid too → new full-bleed slide 3 “The seesaw never stopped.” (jdbbs-public 4f3e67b, 7abf2cb; deck is 27 slides; data-t after it not shifted, +0:40).

Metrics: ~55 % context at handoff; 0 guard bypasses; no subagents.
