...
| **Chapter Opener: Title + Author** | Stacked layout with image behind | ✅ All 9 chapter images render (commits `bfc7967` + `9e621c0`); title + author render | ✅ `.chapter-title`, `.chapter-author` classes (EPUB lines 87–111) — all 9 chapter authors injected via DEV-009 | ✅ both | 2026-05-26: DESIGN-006 closed. Chapters 1-8 silently fixed by DESIGN-005's import-path correction (staging tree had lost JPEG siblings). Intro `ghosts_00_SBAcover.png` separately required dropping its oversized bleed args — Typst silently swallows images whose declared dimensions exceed the page frame. |
...
| **Poem/Verse Block** | Plantin Italic, centered, loose leading | ✅ `poem()` rewritten to body-font italic + centered + 0.8em leading; chapter 1 verse wrapped in `#poem[...]` (commits `bfc7967` + `c2bfcbc`) | ⚠️ Class exists; pandoc doesn't tag verse from docx so it doesn't get applied — EPUB-side fix needs Lua filter work, deferred | Typst ✅, EPUB ⚠️ (deferred) | 2026-05-26: DESIGN-009 closed on Typst side. Verified on `scratch/ghosts-006-009.pdf` page 15. Only chapter 01-soda.typ contains verse; other chapters scanned and have only single-line italic stamps, not verse. EPUB-side verse handling deferred to a separate ticket (pandoc class-markup). |
...
**Document Status:** Matrix closed 2026-05-26. **All 6 child tickets closed 2026-05-26 (latest):** DESIGN-005/007/008 + DEV-013 in commit `1dc4ad6` and `bf878da`; DESIGN-006 (chapter images) in commits `bfc7967` + `9e621c0`; DESIGN-009 (poem) in `bfc7967` + `c2bfcbc`. **TRK-DESIGN-001 is closed.** Ghosts is release-confident on visual quality.
...
| 2 | **TRK-DESIGN-006** ✅ done `bfc7967`+`9e621c0` | P2 | Chapter opener images do not render in compiled Typst output. Chapters 1-8 silently fixed by DESIGN-005's import-path correction (staging tree had lost JPEG siblings). Intro `ghosts_00_SBAcover.png` separately fixed by dropping its oversized bleed args — Typst silently swallows images larger than the page frame. |
...
| 5 | **TRK-DESIGN-009** ✅ done `bfc7967`+`c2bfcbc` | P2 | Poem/verse blocks render as plain justified body in Typst. Fixed: (a) `poem()` rewritten to body-font italic + centered + 0.8em leading, (b) chapter 1 verse wrapped in `#poem[...]` with explicit line breaks. Required importing `poem` at the top of `01-soda.typ` since `#include`'d files don't share scope. |
| 6 | **TRK-DEV-013** ✅ done `bf878da` | P3 | `epubcheck` reports PKG-005 error on generated EPUBs. Root cause was Go's `archive/zip` writer (not pandoc); added `stripMimetypeExtraField` in `srv/epub.go`. |
