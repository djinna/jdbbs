...
- [x] 5.15 Images (from 0.11): print build converts images to grey with tonal auto-level; EPUB keeps colour; spec switch for colour interiors; Inspect reports “N colour images converted for print”. Add-on later: tuned per-image grey conversion + proof sheet, ~$100. Landed 41351b2; add-on still to come

  > **shelley** · 2026-09-18 17:54 UTC  
  > Landed and live (41351b2). Right, Ghosts has no colour images, so I made a three-image test book — a colour photograph, one the author had already converted to grey, and a white chart with a red and a teal line — and ran it through the real pipeline on a free slot. Print PDF: photo came out grey with the full tonal range, the already-grey one untouched byte for byte, the chart’s two lines became a dark grey and a mid grey. EPUB: all three exactly as uploaded, colour intact. Two files are kept, as you assumed — the factory only converts its own working copy on the way to the PDF; your originals stay in the Word file and the EPUB.
  >
  > Three visible pieces: (1) Inspect now labels each image “(colour)” or “(black and white)” and the factory page says “N are colour: kept in the EPUB, converted to grey for the print PDF”; (2) the Illustrations section on the transmittal has the paragraph you asked for, plus a “Colour interior” checkbox for the rare colour-printed book (off by default, goes into the spec); (3) already-grey images are never touched, so the DIY path is simply “place your grey version in Word”. One detail worth knowing: the colour test is share-of-coloured-pixels, not average, so a mostly-white chart with thin coloured lines counts as colour — an average test missed it on the first try. Ready for the book you are processing; I’d like to see its Inspect result.
...
- [x] 0.11 New: in one of the projects we're going to set up today to test the system end-to-end, the images are a mix of black and white and color. In my training, we would insist that people go turn their color images into black and white for the print book, but in this new day and this new model, maybe that's not really needed.  For example, is it fine to include color images in EPUBs, even if most displays will be black and white, like an actual Kindle Paperwhite? Also, when converting color to black and white, usually you want to take care to get the tonal range tuned nicely. Is that something we want to consider doing on the fly? Are there good tools for that that you can grab and use, and do we either just do it or turn it into an add-on? In other words, encourage them to do it themselves, but if they don't want to, we add it as an add-on for $100 or something like that. What do you think?  ·  _added Fri 17:39 UTC_
  ...
  > **jenna** · 2026-09-18 17:47 UTC  
  > Nice, we already have ImageMagick ready to go. And sounds good, the factory can maintain and keep track of two versions of each image, I assume, pretty trivially: one for the PDF that's been converted and the original for the epub. 
  > > I can land it today with Ghosts as the test
  > Do you have color images for Ghost? I don't think so. If not, just build the flow now and I can test on the book we'll be processing in a little while. Do update our ms transmittal page with some of this nice material you just shared, eg 
  > > A plain grey conversion is flat; a grey conversion with an auto-level and a gentle S-curve is what a careful operator would do first on most photos, and it is fine for 80 % of images. What it cannot do is the judgement call — mixing the red channel up to lift a face out of foliage, or noticing that a chart’s two lines became the same grey.
