...
### With style-aware filter (enhanced)
```bash
...
### Edge-case review + decisions hook
```bash
# 1) Generate review artifacts (HTML + detection JSON)
./scripts/build.sh review manuscript.docx

# 2) In the HTML report: set Keep/Strip/Convert, then export edge_case_decisions.json

# 3) Apply decisions during conversion (manual-list hook)
./scripts/build.sh convert manuscript.docx src/book.typ edge_case_decisions.json
```

### Full pipeline to PDF
```bash
# Convert and compile (optionally with decisions)
./scripts/build.sh full manuscript.docx edge_case_decisions.json
```
...
│   └── book.epub
└── scripts/
    ├── docx-to-typst-enhanced.lua # Canonical Pandoc filter
    ├── detect-edge-cases.py       # Edge-case review report generator
    └── build.sh                   # Build + review orchestration
```

## 4. Iterative Workflow

1. **Edit in Word** - Apply styles consistently as you copyedit
2. **Review edge cases** - `./scripts/build.sh review manuscript.docx`
3. **Convert** - Run `./scripts/build.sh convert ... [edge_case_decisions.json]`
4. **Preview** - Compile PDF, check typography
5. **Adjust** - Either fix in Word and reconvert, or hand-edit Typst for final tweaks
